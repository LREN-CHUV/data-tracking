...
ON (nifti.repetition_id=repetition.id AND repetition.sequence_id=sequence.id AND sequence.sequence_type_id=sequence_type.id AND sequence.session_id=session.id AND session.scan_id=scan.id AND scan.participant_id=participant.id) 
